...
const checkStock = tool({
  name: "check_stock",
  description:
    "Check whether a product is in stock by looking it up in the product catalog. " +
    "The catalog is stored in English, so always translate the product name to English before " +
    "searching (e.g. 'micrófono' -> 'microphone').",
  inputSchema: z.object({
    product_name: z.string().describe("The product to check, in English, e.g. 'guitar'"),
  }),
  callback: async ({ product_name }) => {
    const resp = await ddb.send(new ScanCommand({ TableName: process.env.PRODUCTS_TABLE }));
    const matches = resp.Items.filter((item) =>
    ...
    const results = matches.map((item) => {
      const inStock = item.stock !== undefined ? Number(item.stock) > 0 : null;
      if (inStock === null) return `${item.name}: stock information unavailable.`;
      return `${item.name}: ${inStock ? `in stock (${item.stock} available)` : "out of stock"}.`;
    });
    return results.join("\n");
  },
});
